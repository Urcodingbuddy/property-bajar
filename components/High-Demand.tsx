...
const items: Item[] = Array.from({ length: 12 }).map((_, i) => ({
  ...
  postedTime: "3 Month Ago",
  interestedVisitors: 10,
  isFeatured: true,
  image: "./villa.jpg",
}));
  ...
  return (
    <div className="relative w-full max-w-7xl px-4 py-8 flex flex-col gap-10">
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold text-black">HIGH-DEMAND PROJECT TO INVEST NOW</h2>
        <p className="text-sm text-gray-500">The most searched projects in Bhopal</p>
      </div>
      ...
      "
            >
              <div className="h-90 w-90 bg-white rounded-b-3xl shadow-2xl">
                <div className="relative w-full h-60">
                  ...
                  <div className="absolute -bottom-6 right-4 text-xs text-[#707070]">
                    Interested Visitor ({item.interestedVisitors})
                  </div>
                </div>
                ...
                <div className="w-full rounded-3xl h-40 -mt-6 flex flex-col gap-2 justify-center px-4 bg-white relative">
                  ...
                  <p className="text-sm text-[#707070] mt-2">{item.postedBy}</p>
                  <p className="text-sm text-[#707070] mt-1">{item.postedTime}</p>
                </div>
              </div>
            </div>
          ))}
